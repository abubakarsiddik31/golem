...
// rates for a model. Anthropic usage reports cache reads and writes
// beside the input total, so Cost bills Input at the input rate and adds
// CacheRead and CacheWrite at their own rates; a zero cache rate prices
// that cache field's extra tokens at zero. Rates are a snapshot the
// application owns — verify them against the provider's current price
// list.
type Price struct {
	...
	// zero prices cache writes at zero.
