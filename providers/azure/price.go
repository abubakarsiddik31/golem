...
// Price prices usage with per-million-token USD rates, e.g. the rates
// for a deployment's model. Azure OpenAI usage reports cached input
// inside the input total, so Cost bills Input−CacheRead at the input
// rate and CacheRead at the cache-read rate; reasoning tokens are part
// of billed output. Rates are a snapshot the application owns — verify
